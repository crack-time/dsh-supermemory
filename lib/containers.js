...
export async function fetchProfile(scope, containerTag) {
    ...
    // Dynamic facts (isStatic:false) are deliberately excluded here: the
    // per-message recall block re-searches the store on every message and is
    // always fresher, so a session-start snapshot of recent dynamics would be
    // stale AND duplicate the dynamic injection. Keep only the stable facts.
    if (stat.length > 0)
