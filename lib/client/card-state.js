...
export function useSupermemoryCard(deps) {
    ...
    async function load() {
        ...
        try {
            ...
            setServer({
                ...
                recallThreshold: typeof cfg.recallThreshold === 'number' ? cfg.recallThreshold : 0.55,
            });
            setStatus(null);
    ...
    async function commit() {
        ...
        if (reviewProxyPath.trim() !== (server?.reviewProxyPath ?? ''))
            patch.reviewProxyPath = reviewProxyPath.trim();
        if (reviewProxyPort !== (server?.reviewProxyPort ?? 6768))
            patch.reviewProxyPort = reviewProxyPort;
        ...
        if (recallThreshold !== (server?.recallThreshold ?? 0.55))
            patch.recallThreshold = recallThreshold;
