 ...
 */
function searchMessage(scope, session, norm, cfg) {
    const state = recallState(session.id);
    if (state.searched.has(norm))
        return state.bySignature.get(norm) ?? [];
    ...
    if (found.length > 0)
        state.bySignature.set(norm, found);
    return found;
}
 ...
 */
function appendRecallMessage(scope, session, hits) {
    const cfg = resolveConfig(scope);
...
export function registerSessionHooks(ctx, scope) {
    ...
    disposers.push(ctx.on('session/event', (session, event) => {
        ...
        if (event.type === 'user/message') {
            ...
            if (hits.length > 0)
                appendRecallMessage(scope, session, hits);
            return;
        }
