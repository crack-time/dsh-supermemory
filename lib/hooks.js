...
import { isTurnLowValue } from './low-value.js';
...
async function persistTurn(ctx, scope, session, turn, text) {
    ...
    try {
        ...
        if (entry.docId) {
            ...
            if (!res.ok) {
                ctx.logger.warn('supermemory turn PATCH: HTTP ' + res.status + ' ' + (await res.text()).slice(0, 200));
                return;
            }
            ...
            if (!res.ok) {
                ctx.logger.warn('supermemory session create: HTTP ' + res.status + ' ' + (await res.text()).slice(0, 200));
                return;
            }
