...
export function makeForgetTool(scope) {
    return {
        ...
        execute: async (args, exec) => {
            ...
            if (reason)
                body.reason = reason;
            const data = await apiFetch(base, apiKey, '/v4/memories/forget-matching', {
                method: 'POST',
                body,
                signal: exec.signal,
            });
...
export function makeDeleteDocumentTool(scope) {
    return {
        ...
        description: 'Delete supermemory documents AND their produced memories. ' +
        ...
        execute: async (args, exec) => {
            ...
            const docToMemIds = new Map();
            await listDocumentPages(base, apiKey, { limit: 200, maxPages: 10, signal: exec.signal }, (docs) => {
                for (const d of docs) {
                    if (docIdSet.has(d.id))
                        titleMap.set(d.id, (d.title ?? '').slice(0, 80));
                }
            });
            let page = 1;
            let totalPages = 1;
            do {
                const data = await apiFetch(base, apiKey, '/v4/memories/list', {
                    method: 'POST',
                ...
                for (const e of data.memoryEntries ?? []) {
                    for (const dId of e.documentIds ?? []) {
                        ...
                        if (!titleMap.has(dId))
                            titleMap.set(dId, (e.memory ?? '').slice(0, 80));
                    }
                }
            ...
            const allMemIds = [];
            for (const memIds of docToMemIds.values())
                allMemIds.push(...memIds);
            let memForgotten = 0;
            for (let i = 0; i < allMemIds.length; i += 100) {
                const batch = allMemIds.slice(i, i + 100);
                try {
                    const d = await apiFetch(base, apiKey, '/v4/memories/forget-matching', {
                        method: 'POST',
            ...
            for (let i = 0; i < docIds.length; i += 100) {
                ...
                try {
                    const d = await apiFetch(base, apiKey, '/v3/documents/bulk', {
                        method: 'DELETE',
                        body: { ids: batch },
                        signal: exec.signal,
                    });
