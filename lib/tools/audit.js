...
import { apiFetch, docInContainer, listDocumentPages } from '../upstream.js';
/**
 ...
 */
function flagLowValue(content, memCount) {
    const text = (content ?? '').trim();
    const alnum = text.replace(/[^\p{L}\p{N}]/gu, '');
    if (memCount === 0) {
        if (alnum.length === 0)
            return { low: true, reason: 'empty content' };
        if (alnum.length < 30)
            return { low: true, reason: 'short content, no memories extracted' };
    }
    // Greeting-only content with no memories.
    if (memCount === 0 && text.length < 800) {
        const greetingOnly = /^(user:)?\s*(hi+|hello+|hey+|你好|嗨|在吗|嗯|哦)\b/.test(text.replace(/\s+/g, ' '))
            && !/[?？]/.test(text);
        if (greetingOnly)
            return { low: true, reason: 'greeting-only, no memories' };
    }
    return { low: false };
}
/** Fetch a single document's detail (content + memories + status) from upstream. Does not throw. */
async function fetchDocDetail(base, apiKey, docId, signal) {
    try {
        const data = await apiFetch(base, apiKey, '/v3/documents/' + encodeURIComponent(docId), { signal });
        return {
                ...
                await listDocumentPages(base, apiKey, { limit: 200, maxPages: 20, timeoutMs: 15000 }, (pageDocs) => {
                    for (const d of pageDocs) {
                        if (docInContainer(d, tag))
                            docs.push({ id: d.id, customId: d.customId, status: d.status });
                    }
                });
            }
