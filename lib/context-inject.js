...
export function recallConfigOf(scope) {
 ...
 */
export function prewarmRecall(scope, session, container, cfg, content, search = recallSearchSync) {
 ...
 */
export function bindRecall(scope, session, container, cfg, content, search = recallSearchSync) {
    if (!cfg.recallEnabled)
        return;
    const norm = recallSignature(messageText(content));
    if (!norm)
 ...
 */
export function dynamicRecallText(session, cfg) {
    if (!cfg.recallEnabled)
        return '';
