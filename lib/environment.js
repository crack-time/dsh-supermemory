...
function parseProbe(distro, out) {
...
function startProbe(distro) {
    ...
    if (wslSettled.has(distro))
        return; // already have good data this process
    ...
    if (!wslSettled.has(distro) && isHealthy(wslPersisted.get(distro) ?? {})) {
    ...
    const promise = probeWsl(distro)
        ...
        if (isHealthy(wslPersisted.get(distro) ?? {})) {
...
export function environmentBlock(ctx, session) {
    ...
    if (isWslWorkspace(cwd)) {
        ...
        if (!wslSettled.has(distro)
            ...
            && !(isHealthy(wslPersisted.get(distro) ?? {}))) {
        ...
        const probeLayers = [fsProbe, persisted, liveProbe].filter((p) => Boolean(p));
        const probe = Object.assign({}, ...probeLayers.map((p) => (isHealthy(p) ? p : {})));
