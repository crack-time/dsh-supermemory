...
function SupermemorySettingsCard(props) {
	...
	const dirty = server !== null && (baseUrl.trim() !== server.baseUrl || apiKey.trim() !== server.apiKey || serverPath.trim() !== server.serverPath || openaiApiKey.trim() !== server.openaiApiKey || openaiBaseUrl.trim() !== server.openaiBaseUrl || openaiModel.trim() !== server.openaiModel);
	async function load() {
		setLoadErr(false);
		setLoading(true);
		try {
