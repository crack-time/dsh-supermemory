...
function useSupermemoryCard(deps) {
	...
	const dirty = server !== null && (baseUrl.trim() !== server.baseUrl || apiKey.trim() !== server.apiKey || serverPath.trim() !== server.serverPath || openaiApiKey.trim() !== server.openaiApiKey || openaiBaseUrl.trim() !== server.openaiBaseUrl || openaiModel.trim() !== server.openaiModel || recallEnabled !== !!server.recallEnabled || recallTopK !== (server.recallTopK ?? 4));
	async function load() {
		setLoadErr(false);
		setLoading(true);
		try {
...
function SupermemorySettingsCard(props) {
	...
	const { open, baseUrl, apiKey, serverPath, openaiApiKey, openaiBaseUrl, openaiModel, recallEnabled, recallTopK, managed, server, saving, saveFailed, justSaved, testing, status, loadErr, dirty, mgtText, setOpen, setBaseUrl, setApiKey, setServerPath, setOpenaiApiKey, setOpenaiBaseUrl, setOpenaiModel, setRecallEnabled, setRecallTopK, setSaveFailed, commit, runTest } = useSupermemoryCard({
		t: txt,
		applyPatch
	});
