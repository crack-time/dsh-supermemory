...
const CARD_LOCALE = {
	zh: {
		...
		reviewProxyPath: "Review 面板代理 (proxy.mjs)",
		reviewProxyHint: "例：E:Desktopworksupermemory-review-proxyproxy.mjs；留空则不托管",
		reviewProxyPort: "Review 面板端口 (proxy port)",
	...
	en: {
		...
		reviewProxyPath: "Review panel proxy (proxy.mjs)",
		reviewProxyHint: "e.g. E:Desktopworksupermemory-review-proxyproxy.mjs; empty = not managed",
		reviewProxyPort: "Review panel port (proxy port)",
...
*/
const API_BASE = "/plugins/@crack/dsh-supermemory/api";
const API_URLS = {
...
function useSupermemoryCard(deps) {
	...
	const [reviewProxyPath, setReviewProxyPath] = (0, react.useState)("");
	const [reviewProxyPort, setReviewProxyPort] = (0, react.useState)(6768);
	const [recallEnabled, setRecallEnabled] = (0, react.useState)(true);
	const [recallTopK, setRecallTopK] = (0, react.useState)(4);
	const [recallThreshold, setRecallThreshold] = (0, react.useState)(.55);
	...
	const dirty = server !== null && (baseUrl.trim() !== server.baseUrl || apiKey.trim() !== server.apiKey || serverPath.trim() !== server.serverPath || openaiApiKey.trim() !== server.openaiApiKey || openaiBaseUrl.trim() !== server.openaiBaseUrl || openaiModel.trim() !== server.openaiModel || reviewProxyPath.trim() !== (server.reviewProxyPath ?? "") || reviewProxyPort !== (server.reviewProxyPort ?? 6768) || recallEnabled !== !!server.recallEnabled || recallTopK !== (server.recallTopK ?? 4) || recallThreshold !== (server.recallThreshold ?? .55));
	async function load() {
		setLoadErr(false);
		setLoading(true);
		try {
			...
			setRecallThreshold(typeof cfg.recallThreshold === "number" ? cfg.recallThreshold : .55);
			setServer({
				baseUrl: cfg.baseUrl ?? "",
				apiKey: cfg.apiKey ?? "",
				...
				recallThreshold: typeof cfg.recallThreshold === "number" ? cfg.recallThreshold : .55
			});
			setStatus(null);
	...
	async function commit() {
		...
		if (recallThreshold !== (server?.recallThreshold ?? .55)) patch.recallThreshold = recallThreshold;
...
function SupermemorySettingsCard(props) {
	...
	const { open, baseUrl, apiKey, serverPath, openaiApiKey, openaiBaseUrl, openaiModel, reviewProxyPath, reviewProxyPort, recallEnabled, recallTopK, recallThreshold, managed, server, saving, saveFailed, justSaved, testing, status, loadErr, dirty, mgtText, setOpen, setBaseUrl, setApiKey, setServerPath, setOpenaiApiKey, setOpenaiBaseUrl, setOpenaiModel, setReviewProxyPath, setReviewProxyPort, setRecallEnabled, setRecallTopK, setRecallThreshold, setSaveFailed, commit, runTest } = useSupermemoryCard({
		t: txt,
		applyPatch
	});
	...
	return (0, react_jsx_runtime.jsxs)("li", {
		...
		children: [(0, react_jsx_runtime.jsxs)("button", {
			...
			children: [
				...
				(0, react_jsx_runtime.jsxs)("label", {
					className: "sm-settings-row sm-settings-check",
					children: [
						(0, react_jsx_runtime.jsx)("span", {
							className: "sm-settings-label",
							children: txt("recallEnabled")
						}),
						(0, react_jsx_runtime.jsx)("input", {
							type: "checkbox",
						...
						(0, react_jsx_runtime.jsx)("span", {
							className: "sm-settings-hint",
							children: txt("recallEnabledHint")
						})
					]
				}),
				(0, react_jsx_runtime.jsxs)("label", {
				...
				(0, react_jsx_runtime.jsxs)("label", {
					...
					children: [
						...
						(0, react_jsx_runtime.jsx)("span", {
							className: "sm-settings-hint",
							children: txt("recallThresholdHint")
						})
					]
				}),
...
function MemorySpaceBadge(props) {
	...
	return (0, react_jsx_runtime.jsxs)("span", {
		...
		children: [(0, react_jsx_runtime.jsx)(MemoryIcon, {}), container]
	});
}
