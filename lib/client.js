...
function SupermemorySettingsCard(props) {
	...
	const [manageServer, setManageServer] = (0, react.useState)(false);
	...
	const [serverBusy, setServerBusy] = (0, react.useState)(false);
	const [serverActionErr, setServerActionErr] = (0, react.useState)(false);
	...
	const dirty = server !== null && (baseUrl.trim() !== server.baseUrl || apiKey.trim() !== server.apiKey || manageServer !== server.manageServer || serverPath.trim() !== server.serverPath || openaiApiKey.trim() !== server.openaiApiKey || openaiBaseUrl.trim() !== server.openaiBaseUrl || openaiModel.trim() !== server.openaiModel);
	async function load() {
		setLoadErr(false);
		setLoading(true);
		try {
			...
			setStatus(null);
			fetch(HEALTH_URL, { cache: "no-store" }).then((r) => r.json().catch(() => ({}))).then((h) => h?.managed && setManaged(h.managed)).catch(() => {});
		} catch {
			setLoadErr(true);
		} finally {
			setLoading(false);
	...
	async function runTest() {
		...
		try {
			...
			if (data && data.ok) setStatus({
				...
				kind: "err",
				text: txt("checkFailed") + " · " + (data?.error ?? "HTTP " + String(res.status))
			});
		} catch {
			setStatus({
				kind: "err",
				text: txt("checkFailed")
			});
		} finally {
			setTesting(false);
		}
			...
			children: [
				...
				(0, react_jsx_runtime.jsxs)("label", {
					...
					children: [(0, react_jsx_runtime.jsx)("input", {
						...
						className: "sm-settings-label",
						children: txt("manageServer")
					}), (0, react_jsx_runtime.jsx)("span", {
						className: "sm-settings-hint sm-settings-block",
						children: txt("manageServerHint")
					})] })]
				}),
				(0, react_jsx_runtime.jsxs)("label", {
					className: "sm-settings-row",
				...
				(0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						(0, react_jsx_runtime.jsxs)("span", {
							className: cn("sm-settings-status", managed?.state === "running" && "sm-settings-status-ok", (managed?.state === "error" || managed?.state === "missing-exe") && "sm-settings-status-err"),
							children: [
								txt("managedStatus"),
								": ",
								mgtText(managed) ?? "—"
						...
						(0, react_jsx_runtime.jsx)("button", {
							...
							children: serverBusy ? txt("serverBusy") : txt("serverStart")
						}),
						(0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: "sm-settings-test",
							...
							children: serverBusy ? txt("serverBusy") : txt("serverStop")
						})
					]
				}),
				serverActionErr ? (0, react_jsx_runtime.jsx)("p", {
					className: "sm-settings-failed",
					role: "status",
					children: txt("serverActionFailed")
				}) : null,
				(0, react_jsx_runtime.jsxs)("div", {
